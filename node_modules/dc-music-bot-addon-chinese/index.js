...
try {
    if (process.version.slice(1).split('.')[0] < 8) console.error(new Error(`[音樂機器人Musicbot] 請使用 Node v8以上的版本載入插件`));
    ...
    if (!moduleAvailable("ytdl-core") || !moduleAvailable("ytpl") || !moduleAvailable("ytsearcher")) console.error(new Error("[MUSIC]一個或多個模組未找到,請確認已安裝 ytdl-core , ytpl , ytsearcher"));
    ...
    class Music {
      constructor(client, options) {
        ...
        this.play = {
          ...
          help: (options && options.play && options.play.help) || "撥放音樂/播放清單使用歌曲名稱或影片網址",
        ...
        this.help = {
          ...
          help: (options && options.help && options.help.help) || "取得指令幫助",
        ...
        this.pause = {
          ...
          help: (options && options.pause && options.pause.help) || "暫停正在撥放的音樂",
        ...
        this.resume = {
          ...
          help: (options && options.resume && options.resume.help) || "繼續撥放已暫停的音樂",
        ...
        this.leave = {
          ...
          help: (options && options.leave && options.leave.help) || "離開目前的語音頻道",
        ...
        this.queue = {
          ...
          help: (options && options.queue && options.queue.help) || "取得候播的音樂清單",
        ...
        this.np = {
          ...
          help: (options && options.np && options.np.help) || "顯示目前正在撥放的音樂資訊",
        ...
        this.loop = {
          ...
          help: (options && options.loop && options.loop.help) || "循環播放目前的音樂 (單首重複/全部重複)",
        ...
        this.search = {
          ...
          help: (options && options.search && options.search.help) || "從Youtube搜尋音樂(play找不到再使用)",
        ...
        this.clearqueue = {
          ...
          help: (options && options.clear && options.clear.help) || "清空目前的音樂清單",
        ...
        this.volume = {
          ...
          help: (options && options.volume && options.volume.help) || "變更機器人的音量大小",
        ...
        this.remove = {
          ...
          help: (options && options.remove && options.remove.help) || "從後播清單裡移除指定的一首音樂",
          name: (options && options.remove && options.remove.name) || "remove",
          usage: (options && options.remove && options.remove.usage) || "{{prefix}}remove [候播編號]",
        ...
        this.skip = {
          ...
          help: (options && options.skip && options.skip.help) || "跳過當前的音樂或指定的音樂 `skip [候播編號]`",
        ...
        // Cooldown Settings
        this.cooldown = {
          enabled: (options && options.cooldown ? options && options.cooldown.enabled : false),
      ...
      emptyQueue(server) {
        return new Promise((resolve, reject) => {
          if (!musicbot.queues.has(server)) reject(new Error(`${server}未找到任何候播音樂`));
    ...
    musicbot.changeKey = (key) => {
      return new Promise((resolve, reject) => {
        if (!key || typeof key !== "string") reject("YOUTUBE鑰匙必須是字串");
    ...
    client.on("ready", () => {
      console.log(`------- 音樂機器人 -------\n> 插件版本: ${PACKAGE.version}\n> 雙重登錄: ${musicbot.logging}.\n> Node.js 版本: ${process.version}\n Made By: demisex 翻譯:凱恩Kane\n------- Music Bot -------`);
    ...
    musicbot.playFunction = (msg, suffix, args) => {
      if (msg.member.voiceChannel === undefined) return msg.channel.send(musicbot.note('fail', `你不再一個語音頻道裡`));
      if (!suffix) return msg.channel.send(musicbot.note('fail', '請輸入影片名稱或網址!'));
      let q = musicbot.getQueue(msg.guild.id);

      let vc = client.voiceConnections.find(val => val.channel.guild.id == msg.member.guild.id)
      if (vc && vc.channel.id != msg.member.voiceChannel.id) return msg.channel.send(musicbot.note('fail', `你必須和我在同一個語音頻道裡`));
      if (q.songs.length >= musicbot.maxQueueSize && musicbot.maxQueueSize !== 0) return msg.channel.send(musicbot.note('fail', '超出最大候播清單'));
      ...
      if (searchstring.startsWith('http') && searchstring.includes("list=")) {
        msg.channel.send(musicbot.note("search", `努力的解析中...`));
        ...
        ytpl(playid, function(err, playlist) {
          if(err) return msg.channel.send(musicbot.note('fail', `播放清單讀取錯誤`));
          if (playlist.items.length <= 0) return msg.channel.send(musicbot.note('fail', `未從播放清單裡找到任何影片`));
          if (playlist.total_items >= 50) return msg.channel.send(musicbot.note('fail', `超過候播清單所能負荷的數量了(50) (×﹏×)`));
          var index = 0;
          var ran = 0;
          ...
          playlist.items.forEach(video => {
            ...
            if (ran >= playlist.items.length) {
              if (index == 0) msg.channel.send(musicbot.note('fail', `無法從播放清單裡取得任何影片`))
              else if (index == 1) msg.channel.send(musicbot.note('note', `加入了一首音樂!`));
              else if (index > 1) msg.channel.send(musicbot.note('note', `加入了 **${index}** 首音樂!`));
            }
          });
        });
      } else {
        msg.channel.send(musicbot.note("search", `搜尋中: **${searchstring}**`));
        new Promise(async (resolve, reject) => {
          let result = await musicbot.searcher.search(searchstring, { type: 'video' });
          resolve(result.first)
        }).then((res) => {
          if (!res) return msg.channel.send(musicbot.note("fail", "搜尋錯誤,請稍後重試!"));
          ...
          if (msg.channel.permissionsFor(msg.guild.me).has('EMBED_LINKS')) {
            ...
            try {
              embed.setAuthor('已新增到候播清單', client.user.avatarURL);
              ...
              embed.addField("點歌時間", res.queuedOn, musicbot.inlineEmbeds);
              ...
              if (musicbot.requesterName && resMem) embed.setFooter(`看看是誰點了這首歌: ${client.users.get(res.requester).username}`, res.requesterAvatarURL);
              if (musicbot.requesterName && !resMem) embed.setFooter(`看看是誰點了這首歌: \`未知的外星人- (ID: ${res.requester})\``, res.requesterAvatarURL);
              msg.channel.send({
                embed
              });
            } catch (e) {
            ...
            try {
              ...
              .replace(/~/g, '\\~')
              .replace(/`/g, '\\`');
              msg.channel.send(`正在播放: **${songTitle}**\n看看是誰點了這首歌: ${client.users.get(res.requester).username}\n點歌日期: ${res.queuedOn}`)
            } catch (e) {
    ...
    musicbot.helpFunction = (msg, suffix, args) => {
      ...
      if (!suffix) {
        if (msg.channel.permissionsFor(msg.guild.me)
          ...
          embed.setDescription(`輸入 \`${musicbot.botPrefix}${musicbot.help.name} 指令名稱\` 獲得幫助`);
          ...
          var cmdmsg = `= 點歌指令使用大全 =\n輸入 ${musicbot.botPrefix}${musicbot.help.name} [指令] 取得指令的幫助\n`;
          let index = 0;
          let max = musicbot.commandsArray.length;
          for (var i = 0; i < musicbot.commandsArray.length; i++) {
        ...
        if (msg.channel.permissionsFor(msg.guild.me)
          ...
          if (command.exclude) return msg.channel.send(musicbot.note('fail', `${suffix} 未知的指令`));
        ...
        msg.channel.send(musicbot.note('fail', `${suffix} 未知的指令`));
      };
    };

    musicbot.skipFunction = (msg, suffix, args) => {
      if (!msg.member.voiceChannel) return msg.channel.send(musicbot.note('fail', `你不再任何語音頻道裡`));
      const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
      if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', '沒有音樂正在撥放'));
      if (voiceConnection && voiceConnection.channel.id != msg.member.voiceChannel.id) return msg.channel.send(musicbot.note('fail', `你必須和我在同一個語音頻道裡`));

      const queue = musicbot.getQueue(msg.guild.id);
      if (!musicbot.canSkip(msg.member, queue)) return msg.channel.send(musicbot.note('fail', `沒有音樂可以跳過`));

      if (musicbot.queues.get(msg.guild.id).loop == "song") return msg.channel.send(musicbot.note("fail", "當單首音樂連續撥放時無法跳過"));
      ...
      if (!dispatcher || dispatcher === null) {
        ...
        return msg.channel.send(musicbot.note("fail", "當在跳過時發生了錯誤"));
      };
      if (voiceConnection.paused) dispatcher.end();
      dispatcher.end();
      msg.channel.send(musicbot.note("note", "跳過了音樂"));
    };

    musicbot.pauseFunction = (msg, suffix, args) => {
      if (!msg.member.voiceChannel) return msg.channel.send(musicbot.note('fail', `你不再一個語音頻道裡`));
      const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
      if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', 'No music being played.'));
      if (voiceConnection && voiceConnection.channel.id != msg.member.voiceChannel.id) return msg.channel.send(musicbot.note('fail', `你必須和我在同一個語音頻道裡`));
      if (!musicbot.isAdmin(msg.member) && !musicbot.anyoneCanPause) return msg.channel.send(musicbot.note('fail', '你沒有權限暫停音樂ˋ'));

      const dispatcher = voiceConnection.player.dispatcher;
      if (dispatcher.paused) return msg.channel.send(musicbot.note(`fail`, `音樂已經暫停了`))
      else dispatcher.pause();
      msg.channel.send(musicbot.note('note', '播放暫停'));
    };
    ...
    musicbot.resumeFunction = (msg, suffix, args) => {
      ...
      if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', 'No music being played.'));
      if (voiceConnection && voiceConnection.channel.id != msg.member.voiceChannel.id) return msg.channel.send(musicbot.note('fail', `你必須和我在同一個語音頻道裡`));
      if (!musicbot.isAdmin(msg.member) && !musicbot.anyoneCanPause) return msg.channel.send(musicbot.note('fail', `你沒有權限繼續撥放音樂`));

      const dispatcher = voiceConnection.player.dispatcher;
      if (!dispatcher.paused) return msg.channel.send(musicbot.note('fail', `音樂已經在播放`))
      else dispatcher.resume();
      msg.channel.send(musicbot.note('note', '播放繼續'));
    };
    ...
    musicbot.leaveFunction = (msg, suffix) => {
      if (musicbot.isAdmin(msg.member) || musicbot.anyoneCanLeave === true) {
        if (!msg.member.voiceChannel) return msg.channel.send(musicbot.note('fail', `你不再一個語音頻道裡`));
        const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
        if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', '我不再一個語音頻道裡'));
        if (voiceConnection && voiceConnection.channel.id != msg.member.voiceChannel.id) return msg.channel.send(musicbot.note('fail', `你必須和我在同一個語音頻道裡`));
        ...
        msg.channel.send(musicbot.note('note', '完美的從語音頻道離開'));
      } else {
        const chance = Math.floor((Math.random() * 100) + 1);
        if (chance <= 10) return msg.channel.send(musicbot.note('fail', `抱歉 我不能讓你操作這個, ${msg.author.username}.`))
        else return msg.channel.send(musicbot.note('fail', '你沒有權限操作指令'));
      }
    }
    ...
    musicbot.npFunction = (msg, suffix, args) => {
      ...
      if (musicbot.queues.get(msg.guild.id).songs.length <= 0) return msg.channel.send(musicbot.note('note', '候播清單空空如也'));

      if (msg.channel.permissionsFor(msg.guild.me)
        .has('EMBED_LINKS')) {
        const embed = new Discord.RichEmbed();
        try {
          embed.setAuthor('正在播放', client.user.avatarURL);
          ...
          embed.addField("點歌時間", queue.last.queuedOn, musicbot.inlineEmbeds);
          ...
          if (musicbot.requesterName && resMem) embed.setFooter(`看看是誰點了這首歌: ${client.users.get(queue.last.requester).username}`, queue.last.requesterAvatarURL);
          if (musicbot.requesterName && !resMem) embed.setFooter(`看看是誰點了這首歌: \`未知的外星人 (ID: ${queue.last.requester})\``, queue.last.requesterAvatarURL);
          msg.channel.send({
            embed
          });
        } catch (e) {
        ...
        try {
          var songTitle = queue.last.title.replace(/\\/g, '\\\\')
            ...
            .replace(/~/g, '\\~')
            .replace(/`/g, '\\`');
          msg.channel.send(`正在播放: **${songTitle}**\n看看是誰點了這首歌: ${client.users.get(queue.last.requester).username}\n點歌時間: ${queue.last.queuedOn}`)
        } catch (e) {
          console.error(`[${msg.guild.name}] [npCmd] ` + e.stack);
        };
      }
    ...
    musicbot.queueFunction = (msg, suffix, args) => {
      ...
      if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', '沒有音樂正在撥放'));
      if (voiceConnection && voiceConnection.channel.id != msg.member.voiceChannel.id) return msg.channel.send(musicbot.note('fail', `你必須和我在同一個語音頻道裡`));
      if (!musicbot.queues.has(msg.guild.id)) return msg.channel.send(musicbot.note("fail", "沒找到任何候播音樂"));
      else if (musicbot.queues.get(msg.guild.id).songs.length <= 0) return msg.channel.send(musicbot.note("fail", "候播清單空空如也"));
      const queue = musicbot.queues.get(msg.guild.id);
      if (suffix) {
        let video = queue.songs.find(s => s.position == parseInt(suffix) - 1);
        if (!video) return msg.channel.send(musicbot.note("fail", "找不到影片"));
        const embed = new Discord.RichEmbed()
        .setAuthor('候播清單', client.user.avatarURL)
        ...
        .addField("點歌時間", video.queuedOn, musicbot.inlineEmbeds)
        ...
        if (musicbot.requesterName && resMem) embed.setFooter(`看看是誰點了這首歌: ${client.users.get(video.requester).username}`, video.requesterAvatarURL);
        if (musicbot.requesterName && !resMem) embed.setFooter(`看看是誰點了這首歌: \`未知的外星人- (ID: ${video.requester})\``, video.requesterAvatarURL);
        ...
        if (queue.songs.length > 11) {
          ...
          const embed = new Discord.RichEmbed();
          embed.setAuthor('候播音樂', client.user.avatarURL);
          embed.setColor(musicbot.embedColor);
          embed.setFooter(`第${page}頁/共${pages.length}頁`);
          ...
          msg.channel.send(embed).then(m => {
            m.react('⏪').then( r => {
              ...
              forwardsFilter.on('collect', r => {
                ...
                page++;
                embed.setDescription(pages[page - 1]);
                embed.setFooter(`第${page}頁/共${pages.length}頁`, msg.author.displayAvatarURL);
                m.edit(embed);
              })
          ...
          embed.setFooter(`第1頁/共1頁`, msg.author.displayAvatarURL);
    ...
    musicbot.searchFunction = (msg, suffix, args) => {
      ...
      if (!suffix) return msg.channel.send(musicbot.note('fail', '沒找到影片'));
      const queue = musicbot.getQueue(msg.guild.id);
      if (queue.songs.length >= musicbot.maxQueueSize && musicbot.maxQueueSize !== 0) return msg.channel.send(musicbot.note('fail', '達到了影片數量最高限制(50)'));
      ...
      msg.channel.send(musicbot.note('search', `搜尋中: \`${searchstring}\``))
        .then(response => {
          musicbot.searcher.search(searchstring, {
            ...
            .then(searchResult => {
              if (!searchResult.totalResults || searchResult.totalResults === 0) return response.edit(musicbot.note('fail', '未找到搜尋結果'));

              const startTheFun = async (videos, max) => {
                if (msg.channel.permissionsFor(msg.guild.me).has('EMBED_LINKS')) {
                  const embed = new Discord.RichEmbed();
                  embed.setTitle(`選擇你的片`);
                  embed.setColor(musicbot.embedColor);
                  var index = 0;
                  ...
                  embed.setFooter(`看看是誰搜尋了音樂: ${msg.author.username}`, msg.author.displayAvatarURL);
                  msg.channel.send({
                    embed
                  })
                  .then(firstMsg => {
                    ...
                    .then(collected => {
                      ...
                      if (mcon === "cancel") return firstMsg.edit(musicbot.note('note', '搜尋取消'));
                      ...
                      if (song_number >= 0) {
                        ...
                        embed.setAuthor('增加到候播清單', client.user.avatarURL);
                        ...
                        embed.addField("點歌時間", videos[song_number].queuedOn, musicbot.inlineEmbeds);
                        ...
                        if (musicbot.requesterName && resMem) embed.setFooter(`看看是誰點了音樂: ${client.users.get(videos[song_number].requester).username}`, videos[song_number].requesterAvatarURL);
                        if (musicbot.requesterName && !resMem) embed.setFooter(`看看是誰點了音樂: \`未知的外星人- (ID: ${videos[song_number].requester})\``, videos[song_number].requesterAvatarURL);
                        msg.channel.send({
                          embed
                        }).then(() => {
                  ...
                  msg.channel.send(`\`\`\`\n= 輸入音樂編號播放 =\n${vids}\n\n= 輸入 **cancel** 取挑播放 =`).then(firstMsg => {
                    ...
                    .then(collected => {
                      ...
                      if (song_number >= 0) {
                        ...
                        embed.setColor(musicbot.embedColor);
                        embed.addField(videos[song_number].channelTitle, `[${songTitle}](${videos[song_number].url})`, musicbot.inlineEmbeds);
                        embed.addField("點歌日期", videos[song_number].queuedOn, musicbot.inlineEmbeds);
                        if (!musicbot.bigPicture) embed.setThumbnail(`https://img.youtube.com/vi/${videos[song_number].id}/maxresdefault.jpg`);
                        if (musicbot.bigPicture) embed.setImage(`https://img.youtube.com/vi/${videos[song_number].id}/maxresdefault.jpg`);
                        const resMem = client.users.get(videos[song_number].requester);
                        if (musicbot.requesterName && resMem) embed.setFooter(`看看是誰點的歌: ${client.users.get(videos[song_number].requester).username}`, videos[song_number].requesterAvatarURL);
                        if (musicbot.requesterName && !resMem) embed.setFooter(`看看是誰點的歌: \`未知的外星人 (ID: ${videos[song_number].requester})\``, videos[song_number].requesterAvatarURL);
                        msg.channel.send({
                          embed
                        }).then(() => {
                          queue.songs.push(videos[song_number]);
    ...
    musicbot.volumeFunction = (msg, suffix, args) => {
      if (!msg.member.voiceChannel) return msg.channel.send(musicbot.note('fail', `你不再一個語音頻道裡`));
      const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
      if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', '沒有音樂正在播放'));
      if (voiceConnection && voiceConnection.channel.id != msg.member.voiceChannel.id) return msg.channel.send(musicbot.note('fail', `你必須和我在同一個語音頻道裡`));
      if (!musicbot.canAdjust(msg.member, musicbot.queues.get(msg.guild.id))) return msg.channel.send(musicbot.note('fail', `只有管理員或DJ能改變音量`));
      const dispatcher = voiceConnection.player.dispatcher;

      if (!suffix || isNaN(suffix)) return msg.channel.send(musicbot.note('fail', '找不到影片'));
      suffix = parseInt(suffix);
      if (suffix > 200 || suffix <= 0) return msg.channel.send(musicbot.note('fail', '音量超過最大限制(1~200)'));

      dispatcher.setVolume((suffix / 100));
      musicbot.queues.get(msg.guild.id).volume = suffix;
      msg.channel.send(musicbot.note('note', `音量改變為 ${suffix}%.`));
    };

    musicbot.clearFunction = (msg, suffix, args) => {
      if (!musicbot.queues.has(msg.guild.id)) return msg.channel.send(musicbot.note("fail", "沒有候播的音樂"));
      if (!musicbot.isAdmin(msg.member)) return msg.channel.send(musicbot.note("fail", `只有管理員或DJ能清空候播清單`));
      let vc = client.voiceConnections.find(val => val.channel.guild.id == msg.member.guild.id)
      if (vc && vc.channel.id != msg.member.voiceChannel.id) return msg.channel.send(musicbot.note('fail', `你必須和我在同一個語音頻道裡`));
      musicbot.emptyQueue(msg.guild.id).then(res => {
        msg.channel.send(musicbot.note("note", "候播清單空空如也"));
        const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
        if (voiceConnection !== null) {
          const dispatcher = voiceConnection.player.dispatcher;
          if (!dispatcher || dispatcher === null) {
            if (musicbot.logging) return console.log(new Error(`dispatcher null on skip cmd [${msg.guild.name}] [${msg.author.username}]`));
            return msg.channel.send(musicbot.note("fail", "出現了未知的錯誤"));
          };
          if (voiceConnection.paused) dispatcher.end();
          dispatcher.end();
        }
        ...
        return msg.channel.send(musicbot.note("fail", "在清空候播清單時出現了未知的錯誤"));
      })
    };
    ...
    musicbot.removeFunction = (msg, suffix, args) => {
      ...
      let vc = client.voiceConnections.find(val => val.channel.guild.id == msg.member.guild.id)
      if (vc && vc.channel.id != msg.member.voiceChannel.id) return msg.channel.send(musicbot.note('fail', `你必須和我在同一個語音頻道裡`));
      if (parseInt(suffix) - 1 == 0) return msg.channel.send(musicbot.note("fail", "你無法清除正在播放的音樂"));
      let test = musicbot.queues.get(msg.guild.id).songs.find(x => x.position == parseInt(suffix) - 1);
      if (test) {
        if (test.requester !== msg.author.id && !musicbot.isAdmin(msg.member)) return msg.channel.send(musicbot.note("fail", "你無法移除那東東"));
        ...
        musicbot.updatePositions(newq, msg.guild.id).then(res => {
          ...
          msg.channel.send(musicbot.note("note", `已移除:  \`${test.title.replace(/`/g, "'")}\``));
        })
      } else {
        msg.channel.send(musicbot.note("fail", "無法找到影片或出現了未知的錯誤"));
      }
    };
    ...
    musicbot.loopFunction = (msg, suffix, args) => {
      ...
      if (musicbot.queues.get(msg.guild.id).loop == "none" || musicbot.queues.get(msg.guild.id).loop == null) {
        ...
        msg.channel.send(musicbot.note('note', `單曲重播啟動(輸入 ${musicbot.botPrefix}loop切換模式)! :repeat_one:`));
      } else if (musicbot.queues.get(msg.guild.id).loop == "song") {
        musicbot.queues.get(msg.guild.id).loop = "queue";
        msg.channel.send(musicbot.note('note', `全部重播啟動(輸入 ${musicbot.botPrefix}loop切換模式)! :repeat:`));
      } else if (musicbot.queues.get(msg.guild.id).loop == "queue") {
        musicbot.queues.get(msg.guild.id).loop = "none";
        msg.channel.send(musicbot.note('note', `重播關閉(輸入 ${musicbot.botPrefix}loop切換模式)! :arrow_forward:`));
        const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
        const dispatcher = voiceConnection.player.dispatcher;
    ...
    musicbot.loadCommand = (obj) => {
      return new Promise((resolve, reject) => {
        ...
        if (musicbot.logging) console.log(`[音樂機器人] 初始化 ${obj.name} 成功!`);
    ...
    musicbot.executeQueue = (msg, queue) => {
      if (queue.songs.length <= 0) {
        msg.channel.send(musicbot.note('note', '播放完畢'));
        ...
        const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
        if (voiceConnection !== null) return voiceConnection.disconnect(); //離開語音頻道
      };
      ...
      new Promise((resolve, reject) => {
          ...
          if (voiceConnection === null) {
            if (msg.member.voiceChannel && msg.member.voiceChannel.joinable) {
              ...
              msg.channel.send(musicbot.note('fail', '我沒有權限加入你的語音頻道'))
              reject();
            } else {
          ...
          if (!video) {
            ...
            if (!video) {
              msg.channel.send(musicbot.note('note', '播放結束'));
              musicbot.emptyQueue(msg.guild.id);
              const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
...
if (musicbot.messageNewSong == true && queue.last && musicbot.queues.get(msg.guild.id).loop !== "song") {
            ...
            if (msg.channel.permissionsFor(msg.guild.me).has('EMBED_LINKS')) {
              const embed = new Discord.RichEmbed()
              .setTitle("正在播放", `${req !== null ? req.displayAvatarURL : null}`)
              ...
              .setColor(musicbot.embedColor)
              .setFooter(`看看是誰點了這首歌: ${req !== null ? req.username : "未知的外星人"}`, `${req !== null ? req.displayAvatarURL : null}`);
              msg.channel.send({embed});
            } else {
          ...
          try {
            ...
            connection.on('error', (error) => {
              console.error(error);
              if (msg && msg.channel) msg.channel.send(musicbot.note('fail', `連接時出現了錯誤,正在重試`));
              musicbot.executeQueue(msg, musicbot.queues.get(msg.guild.id));
            });

            dispatcher.on('error', (error) => {
              console.error(error);
              if (msg && msg.channel) msg.channel.send(musicbot.note('fail', `播放音樂時出現了錯誤,正在重試`));
              musicbot.executeQueue(msg, musicbot.queues.get(msg.guild.id));
            });
            ...
            dispatcher.on('end', () => {
              setTimeout(() => {
                ...
                if (voiceConnection !== null && voiceConnection.channel.members.size <= 1){
                    msg.channel.send(musicbot.note('note', '沒人在語音頻道裡,先閃6~'))
                    musicbot.queues.set(msg.guild.id, {songs: [], last: null, loop: "none", id: msg.guild.id, volume: musicbot.defVolume});
                    if (musicbot.musicPresence) musicbot.updatePresence(musicbot.queues.get(msg.guild.id), msg.client, musicbot.clearPresence).catch((res) => { console.warn(`[MUSIC] Problem updating MusicPresence`); });
                ...
                if (musicbot.queues.get(msg.guild.id).songs.length > 0) {
                  if (loop == "none" || loop == null) {
                    ...
                    musicbot.updatePositions(musicbot.queues.get(msg.guild.id).songs, msg.guild.id).then(res => {
                      musicbot.queues.get(msg.guild.id).songs = res;
                      musicbot.executeQueue(msg, musicbot.queues.get(msg.guild.id));
                    }).catch(() => { console.error(new Error("在移動音樂時出現了錯誤")); });
                  } else if (loop == "queue" || loop == "song") {
                    musicbot.executeQueue(msg, musicbot.queues.get(msg.guild.id));
                  };
                } else if (musicbot.queues.get(msg.guild.id).songs.length <= 0) {
                  if (msg && msg.channel) msg.channel.send(musicbot.note('note', '播放結束'));
                  musicbot.queues.set(msg.guild.id, {songs: [], last: null, loop: "none", id: msg.guild.id, volume: musicbot.defVolume});
                  if (musicbot.musicPresence) musicbot.updatePresence(musicbot.queues.get(msg.guild.id), msg.client, musicbot.clearPresence).catch((res) => { console.warn(`[MUSIC] Problem updating MusicPresence`); });
                  const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
                  if (voiceConnection !== null) return voiceConnection.disconnect();
...
/*  client.on('guildMemberRemove', member => {
    if(member.guild.id === '475333539957833740'){
      member.guild.channels.get('475861361491181598').send(member.tag + '默默地離開了 **' + member.guild.name + '**');
  }
}); */
}
